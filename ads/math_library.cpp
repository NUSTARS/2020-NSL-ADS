...
namespace nustars {
  ...
  // filter_dif = [alt(n)-alt(n-1), vel(n)-vel(n-1)]
  static vector<double> filter(vector<double>& sensor_data, vector<double>& filter_data, vector<double>& fitler_dif){
    ...
    double new_acc = wgt_acc*(fil_acc + dif_vel*dt) + get_conj(wgt_acc)*sen_acc;
    double new_vel = fil_vel + dt*(wgt_vel*fil_acc + get_conj(wgt_vel)*dif_alt);
